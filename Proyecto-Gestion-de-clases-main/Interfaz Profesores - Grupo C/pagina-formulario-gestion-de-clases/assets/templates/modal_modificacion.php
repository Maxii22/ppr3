    ...
    <form method="post">
      <label for="new-materia-id">
        ...
        <select class="inputs-modal mod_materia" name="new_materia" id="new-materia-id">
          <option hidden value="<?php echo $id_materia;?>" class="op-materia " id="op-principal-materia"><?php echo $materia ;?></option>
          <?php
      ...
      <label for="aula-id">
        Aula:
        <input type="number" min="1" max=100 placeholder="Ingrese número de aula" class="inputs-modal" name="new_aula" id="aula-id" value="<?php echo $aula; ?>">
      </label>

      <label for="new-hora-id">
        Hora
        <select class="inputs-modal mod_hora" name="new_hora" id="new-hora-id">
          <option hidden value="<?php echo $hora; ?>" class="op-hora "><?php echo $hora; ?></option>
...
if (isset($_POST['btn_Modificar'])) {
  ...
  if (
    ...
    $consulta = "UPDATE CLASES SET CODIGO_MATERIA = '$materia', FECHA ='$fecha', HORA = '$hora', TEMAS = '$temas', NOVEDADES = '$novedad', COMISION = '$comision', AULA = '$aula', ARCHIVOS = '$archivos' 
    ...
    if ($resultado) {
      ...
      <div class="flex min-h-screen items-center justify-center bg-transparent ">
        <div class="rounded-lg bg-gray-50 px-16 py-14">
          ...
          <p class="w-[230px] text-center font-normal text-gray-600">¡La clase se modifico correctamente!</p>
        </div>
      </div>
